...
class SNRender: SolidNativeModule {
    ...
    override func getJSValueRepresentation() -> JSValue {
        ...
        builder.addSyncFunction("setProp") {
        ...
        builder.addSyncFunction("removeChild") {
            (_ id: String, childId: String) in
            let view = self.viewManager.getViewById(id)
            let viewChild = self.viewManager.getViewById(childId)

            print(
                "[del] parent: \(view.getName()) \(viewChild.parentElement?.getName()), ele: \(viewChild.getName())"
            )
            view.removeChild(viewChild)
            self.viewManager.removeViewById(childId)
        }

        builder.addSyncFunction("insertBefore") {
        ...
        builder.addSyncFunction("_webView_load") {
            (_ id: String, _ urlString: String) in
            guard
                let node = self.viewManager.createdViewRegistry[id]
                    as? SNWebView
            else { return }
            var url: URL
        ...
        builder.addSyncFunction("_webView_loadHTMLString") {
        ...
        builder.addSyncFunction("_webView_evaluateJavaScript") {
            (_ id: String, _ code: String, _ callback: JSValue) in
            guard
                let webView =
                    (self.viewManager.createdViewRegistry[id] as? SNWebView)?
