...
class SNNavigationStack: SolidNativeView {
    ...
    struct SNNavigationStack: View {
        ...
        init(props: SolidNativeProps, owner: SolidNativeView) {
            self.props = props
            if let newVal = props.getPropAsJSValue(name: "path"), newVal.isArray == true, let val = newVal.toArray() as? [String], let destFunc = props.getPropAsJSValue(name: "navigationDestination") {
                var newPath = [NavigationRouteParam]()
                for i in 0..<val.count {
                    let name = val[i]
                    if let node = destFunc.call(withArguments: [name]), node.isObject, let id = node.forProperty("id").toString() {
        ...
        func onChangeByNative(oldVal: [NavigationRouteParam], newVal: [NavigationRouteParam]) {
        ...
        var body: some View {
            return NavigationStack(path: $path) {
                ...
                .onChange(of: props.getPropAsJSValue(name: "path")) { oldVal, newVal in
                    guard newVal?.isArray == true, let val = newVal?.toArray() as? [String], let destFunc = props.getPropAsJSValue(name: "navigationDestination") else { return }
                    var breakIdx = 0
