...
class SNSlider: SolidNativeView {
    ...
    struct SNSlider: View {
        ...
        func onEditingChanged(editing: Bool) {
            props.callCallbackWithArgs(name: "onEditingChanged", args: [editing])
        }

        var body: some View {
            let maxValue = props.getNumber(name: "maxValue", default: 100).doubleValue
            let minValue = props.getNumber(name: "minValue", default: 0).doubleValue
